...
class HfssDesign(COMWrapper):
    ...
    def eval_var_str(self, name, unit=None):  # can only parse 2 times for now
        ...
        try:
                ...
                eval(
                    str(
                        sympy_parser.parse_expr(
                            str(sympy_parser.parse_expr(str(name), self.variables)),
                            self.variables,
                        )
                    )
                )
            ...
            msg = (
                "Parsed expression contains a string which does "
...
class HfssModeler(COMWrapper):
    ...
    def connect_faces(self, entity1, entity2):
        name = self._modeler.Connect(
            ["NAME:Selections", "Selections:=", ",".join([entity1.name, entity2.name])]
        )
        return name
    ...
    def create_coor_sys(self, coor_sys="chip", rel_coor=None, ref_name="Global"):
        if not (ref_name in self._modeler.GetCoordinateSystems()):
            raise ValueError(
                "%s reference coordinate system does \
                             not exist"
                % ref_name
            )
        else:
            ...
            self._modeler.ChangeProperty(
                [
                    ...
                    [
                        ...
                        [
                            "NAME:ChangedProps",
                            ["NAME:Reference CS", "Value:=", ref_name],
                            [
                                "NAME:Origin",
                                "X:=",
                                str(origin[0]),
...
Still on progress
    ...
    def assign_terminal_auto(self, entity, name, ground):
        ...
        self._boundaries.AutoIdentifyTerminals(groundarray, name, False)
    ...
    def assign_mesh_length(self, entities, length):
        ...
        if all(list_bool):
            ...
            raise TypeError(
                "Please do not mix 3D element with other \
    ...
    def assign_material(self, entities, material):
        ...
        self._modeler.AssignMaterial(
            self._selections_array(*entities_name),
            [
                "NAME:Attributes",
                "MaterialValue:=",
                material,
...
def get_desktop():
    ...
    if not is_admin:
        print(
            "\033[93m WARNING: you are not runnning as an admin! You need to run as an admin. You will probably get an error next. \033[0m"
        )
